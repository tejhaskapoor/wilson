...
def beta(C, HIGHSCALE=1, newphys=True):
    ...
    #just adding random terms for Gn copying from Ge - s
    Beta["Gn"] = 3/2*Gn @ Gn.conj().T @ Gn + (GammaH \
      - 3/4*(3*g**2 + 5*gp**2))*Ge + 2*m2/HIGHSCALE**2*3*C["ephi"] 
    ...
    Xin = 2*(my_einsum("prst,rs", C["qu1"], Gu) \
      + 4/3*my_einsum("prst,rs", C["qu8"], Gu)) \
      - (3*my_einsum("ptsr,sr", C["quqd1"], np.conj(Gd)) \
      + 1/2*(my_einsum("stpr,sr", C["quqd1"], np.conj(Gd)) \
      + 4/3*my_einsum("stpr,sr", C["quqd8"], np.conj(Gd)))) \
    ...
    #RRRR operators
    Beta["nd"] = (4/3)*Nc*yd**2*gp**2*my_einsum("prww,st", C["nd"], I3)\
    ...
    Beta["nu"] = (4/3)*Nc*yu*yd*gp**2*my_einsum("prww,st", C["nd"], I3) \
    ...
    Beta["ne"] = (4/3)*Nc*ye*yd*gp**2*my_einsum("prww,st", C["nd"], I3) \
    ...
    Beta["nn"] = - my_einsum("pr,st", Gn.conj().T @ Gn, C["phin"]) \
    ...
    Beta["nedu"] =((yd-yu)**2 + ye*(ye + 8*yu - 2*yd))*gp**2*my_einsum("prst",C["nedu"])\
    ...
    Beta["qn"] =(4/3)*Nc*yq*yd*gp**2*my_einsum("stww,pr", C["nd"], I3) \
    ...
    Beta["ln"] = (4/3)*Nc*yl*yd*gp**2*my_einsum("stww,pr", C["nd"], I3) \
    ...
    Beta["lnle"] = ((ye**2 - 8*ye*yl + 6*yl**2)*gp**2 - (3/2)*g**2)*my_einsum("prst", C["lnle"]) \
    ...
    Beta["lnqd1"] = ((yd**2 - 2*yd*(yl + 4*yq) + (yl + yq)**2)*gp**2 - 8*gs**2)*my_einsum("prst", C["lnqd1"]) \
    ...
    Beta["lnqd3"] = (-(1/2)*yl*(yd+yq)*gp**2 + (3/8)*g**2)*my_einsum("prst",C["lnqd1"])\
    ...
    Beta["lnuq"] = (((yl+yu)**2 + yq*(yq - 2*yl -8*yu))*gp**2 - 8*gs**2)*my_einsum("prst",C["lnuq"]) \
...
#psi2phi3 tems
    Beta["nphi"] = -(9*yl**2*gp**2 + (27/4)*g**2)*my_einsum("pr",C["nphi"]) \
    ...
    Beta["phin"] = (4/3)*yh**2*gp**2*my_einsum("pr",C["phin"])\
    ...
    Beta["nW"] = ((3*CF2 - b02)*g**2 - 3*yl**2*gp**2)*my_einsum("pr",C["nW"])\
      +3*yl*gp*g*my_einsum("pr",C["nB"])

    Beta["nB"] = (-3*CF2*g**2 + (3*yl**2 - b01)*gp**2)*my_einsum("pr",C["nB"]) \
      +12*CF2*yl*gp*g*my_einsum("pr",C["nW"])

    Beta["phine"] = -3*ye**2*my_einsum("pr", C["phine"])*g**2
