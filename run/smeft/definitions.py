...
def flavor_rotation(C_in, Uq, Uu, Ud, Ul, Ue, sm_parameters=True):
    ...
    if sm_parameters:
        ...
        C['Gn'] = np.eye(3).conj().T @ C_in['Gn'] @ np.eye(3) #
    ...
    C['nphi'] = np.eye(3) @ C_in['nphi'] @ np.eye(3)#
    C['phin'] = np.eye(3) @ C_in['phin'] @ np.eye(3)#
    C['phine'] = np.eye(3) @ C_in['phine'] @ np.eye(3)#
    # 4-fermion
    ...
    C['ln'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['ln']) #
    C['qe'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', Uq, Ue, Uq.conj(), Ue.conj(), C_in['qe'])
    C['qn'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['qn'])#
    C['ed'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', Ue, Ud, Ue.conj(), Ud.conj(), C_in['ed'])
    C['nd'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['nd']) #
    C['nu'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['nu']) #
    C['ne'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['ne']) #
    C['nn'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['nn']) #
    ...
    C['nedu'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['nedu']) #
    C['lnle'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['lnle'])#
    C['lnqd1'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['lnqd1'])#
    C['lnqd3'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['lnqd3'])#
    C['lnuq'] = np.einsum('jb,ld,ia,kc,ijkl->abcd', np.eye(3), np.eye(3), np.eye(3), np.eye(3), C_in['lnuq'])#
